...
#        assert type(cxn) is npy.core.defmatrix.matrix, "cxnmatrix must be a numpy matrix"
        ...
        nextstate = apply_gate( npy.dot( current_state.flatten(), cxn ) )
...
#	if ei_across_partition > self.numunits:
#		print "parts=%s" % str(parts)
#		print "x1=%s" % x1
#		sumparts = (2**(-self.x1_STATS[x1]['H_X0_given_x1']) * Sum_x0_in_X0)
#		print "sumparts=%s" % sumparts
#		raw_input('...')
...
#            print "ei(%s/P=%s)=%s" % ( x1, str(parts), ei )
...
#        print "%s: running parts=%s" % (perturb, str(parts))
        ...
        #               = N - H[X0|X1]
        if parts is None or max( [ len(part) for part in parts ] ) == self.numunits:
...
#        print term.RED + "\t%s=%s" % (part, H_M0_given_M1) + term.NORMAL        
    ...
    def average_phi( self ):
        """Returns the raw average of phi by calculating the phi of all x1 states."""
        total = 0.0
        for x1 in self.VALID_OUTPUTS:
            prob_x1 = self.x1_STATS[x1]['prob_this_x1']
            phi_x1, MIP_x1 = self.PHI( x1 )
            total += prob_x1 * phi_x1

        return total
    
    def average_MIP( self, partitionsfilename=None, allpartitions=False, perturb='STATES' ):
        ...
        if allpartitions:
#            print "partitionsfilename=%s" % partitionsfilename
            global BELL_biglist
            BELL_biglist = []
            BELL_all_partitions( [], range(self.numunits) )
            print "Attempting %s partitions..." % len(BELL_biglist)
...
#    tm=cxnmatrices.g3_1d
    ...
    for x1 in Vphi.VALID_OUTPUTS:
        x1state = Vphi._dec2binstate( x1 ).flatten()
        ...
        vphi, vMIP = Vphi.PHI(x1, perturb='STATES')
        vphi = round(vphi,5)
